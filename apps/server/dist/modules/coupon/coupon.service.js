...
class CouponService extends service_1.BaseService {
    constructor() {
        super("coupons");
    }
    async findAll() {
        ...
        return coupons.map((c) => ({
            ...c,
            _count: {
                orders: usageCounts.find((u) => u.coupon_code === c.code)?._count
                    .coupon_code || 0,
            },
        }));
    }
    ...
    async validateCoupon(code, userId, subtotal, userSegmentId) {
        ...
        if (!coupon) {
            throw appError_1.AppError.notFound("Kupon tidak ditemukan");
        }
        // 1. Check expiration
        if (coupon.expires_at && new Date() > coupon.expires_at) {
            throw appError_1.AppError.badRequest("Kupon sudah kadaluarsa");
        }
        ...
        if (coupon.usage_limit) {
            ...
            if (usedCount >= coupon.usage_limit) {
                throw appError_1.AppError.badRequest("Kuota kupon sudah habis");
            }
        }
        ...
        if (coupon.usage_limit_per_user) {
            ...
            if (userUsedCount >= coupon.usage_limit_per_user) {
                throw appError_1.AppError.badRequest("Anda sudah mencapai batas penggunaan untuk kupon ini");
            }
        }
        // 3. Check segment restriction
        const restrictedSegments = coupon.segment_coupons.map((sc) => sc.segment_id);
        if (restrictedSegments.length > 0) {
            if (!userSegmentId || !restrictedSegments.includes(userSegmentId)) {
                throw appError_1.AppError.badRequest("Kupon ini tidak berlaku untuk segmen member Anda");
            }
        }
    ...
    async getAvailableCoupons(userSegmentId, userId) {
        const now = new Date();
        const coupons = await this.db.coupons.findMany({
            where: {
                AND: [
