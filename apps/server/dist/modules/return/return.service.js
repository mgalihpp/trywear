...
class ReturnService extends service_1.BaseService {
    notificationService;
    constructor() {
        super("returns");
    ...
    findById = async (id) => {
        ...
        if (!returnData) {
            throw appError_1.AppError.notFound("Return tidak ditemukan");
        }
        return returnData;
    };
    /**
     ...
     */
    createReturn = async (input, userId) => {
        ...
        if (!order) {
            throw appError_1.AppError.notFound("Pesanan tidak ditemukan");
        }
        // 2. Check if order status allows return (must be delivered)
        if (order.status !== "delivered" && order.status !== "completed") {
            throw appError_1.AppError.badRequest("Hanya pesanan yang sudah diterima yang dapat dikembalikan");
        }
        ...
        if (shipment?.delivered_at) {
            ...
            if (daysDiff > 7) {
                throw appError_1.AppError.badRequest("Batas waktu pengajuan pengembalian adalah 7 hari setelah pesanan diterima");
            }
        }
        // 3. Check if return already exists for this order
        const existingReturn = order.returns.find((r) => r.status !== "rejected" && r.status !== "completed");
        if (existingReturn) {
            throw appError_1.AppError.badRequest("Sudah ada pengajuan pengembalian untuk pesanan ini");
        }
        // 4. Validate items belong to order
        for (const item of input.items) {
            const orderItem = order.order_items.find((oi) => oi.id === item.order_item_id);
            if (!orderItem) {
                throw appError_1.AppError.badRequest(`Item dengan ID ${item.order_item_id} tidak ditemukan dalam pesanan`);
            }
            if (item.quantity > orderItem.quantity) {
                throw appError_1.AppError.badRequest(`Jumlah pengembalian melebihi jumlah pembelian untuk item ${orderItem.title}`);
            }
        }
     ...
     */
    updateStatus = async (returnId, input) => {
        ...
        if (updatedReturn.user_id) {
            const notificationMap = {
                approved: () => this.notificationService.create({
                ...
                rejected: () => this.notificationService.create({
                    user_id: updatedReturn.user_id,
                ...
                completed: () => this.notificationService.create({
                    user_id: updatedReturn.user_id,
