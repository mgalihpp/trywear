...
class SegmentService extends service_1.BaseService {
    constructor() {
    ...
    async findById(id) {
        ...
        if (!segment) {
            throw appError_1.AppError.notFound("Segment not found");
        }
        return segment;
    }
    /**
    ...
    async create(data) {
        ...
        if (existing) {
            throw appError_1.AppError.badRequest("Segment with this slug already exists");
        }
    ...
    async update(id, data) {
        ...
        return await this.db.customerSegment.update({
            ...
            data: {
                ...data,
                min_spend_cents: data.min_spend_cents !== undefined
                    ? BigInt(data.min_spend_cents)
                    : undefined,
                max_spend_cents: data.max_spend_cents !== undefined
     ...
     */
    async calculateUserSpending(userId) {
     ...
     */
    async findSegmentBySpending(spendingCents) {
        ...
        for (const segment of segments) {
            ...
            const maxMet = segment.max_spend_cents === null ||
     ...
     */
    async assignSegmentToUser(userId) {
     ...
     */
    async getCustomersBySegment(segmentSlug, page = 1, limit = 20) {
