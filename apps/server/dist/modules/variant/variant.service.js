...
class ProductVariantsService extends service_1.BaseService {
    inventoryService;
    constructor() {
    ...
    async create(data) {
        const { sku, product_id, additional_price_cents, option_values, stock_quantity, reserved_quantity, safety_stock, } = data;
        ...
        const createdInventory = await this.inventoryService.createOrUpdate(createdVariant.id, {
    ...
    async createMany(data) {
        const createdVariants = await Promise.all(data.map(async (item) => {
            const createdVariant = await this.db[this.model].create({
                data: {
                    sku: item.sku,
    ...
    async getAvailableColors() {
        ...
        const colors = [
            ...new Set(variants
                ...
                if (typeof v.option_values === "object" &&
