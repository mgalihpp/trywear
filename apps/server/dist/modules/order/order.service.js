...
class OrderService extends service_1.BaseService {
    notificationService;
    couponService;
    constructor() {
        super("orders");
    ...
    create = async (input, idempotencyKey) => {
        ...
        if (variants.length !== variantIds.length) {
            throw appError_1.AppError.badRequest("Some variants not found");
        }
        ...
        const subtotal = itemsDetailed.reduce((s, it) => s + it.total_price_cents, 0);
        function shippingRate(shipment_method_id) {
            let basePrice;
        ...
        const discount = await (async () => {
            ...
            if (user?.segment?.discount_percent) {
                const segmentDiscount = Math.round(subtotal * (user.segment.discount_percent / 100));
                totalDiscount += segmentDiscount;
            }
            // 2. Coupon Discount
            if (input.coupon_code) {
                try {
                    const { discountAmount } = await this.couponService.validateCoupon(input.coupon_code, input.user_id, subtotal, user?.segment_id);
        ...
        const result = await this.db.$transaction(async (tx) => {
            ...
            for (const it of itemsDetailed) {
                ...
                if (!inv)
                    throw appError_1.AppError.badRequest(`Inventory missing for ${it.variant_id}`);
                if (inv.stock_quantity - inv.reserved_quantity < it.quantity) {
                    throw appError_1.AppError.badRequest(`Insufficient stock for variant ${it.variant_id}`);
                }
        ...
        const categoryMap = Object.fromEntries(categories.map((cat) => [cat.id, cat.name ?? `Category ${cat.id}`]));
        const itemDetailsForMidtrans = result.order.order_items.map((oi) => {
            const detailed = itemsDetailed.find((d) => d.variant_id === oi.variant_id);
            return {
                ...
                brand: "TryWear",
                category: detailed.category_id != null
        ...
        const snapPayload = {
            ...
            callbacks: {
                finish: `${appConfig_1.default.CLIENT_ORIGIN}/order?order_id=${result.order.id}`,
                error: `${appConfig_1.default.CLIENT_ORIGIN}/order?order_id=${result.order.id}`,
            },
            gopay: {
                callback_url: `${appConfig_1.default.CLIENT_ORIGIN}/order?order_id=${result.order.id}`,
            },
        };
        ...
        await this.notificationService.notifyOrderCreated(input.user_id, result.order.id);
    ...
    updateStatus = async (orderId, input) => {
        return await this.db.$transaction(async (tx) => {
            ...
            if (!order) {
                throw appError_1.AppError.notFound("Order tidak ditemukan");
            }
            ...
            const shouldDecreaseStock = oldStatus !== "shipped" &&
                oldStatus !== "delivered" &&
                newStatus === "shipped";
            if (shouldDecreaseStock) {
                for (const item of order.order_items) {
                    ...
                    const currentReserved = variant.inventory?.[0]?.reserved_quantity ?? 0;
                    if (currentStock < item.quantity) {
                        throw appError_1.AppError.badRequest(`Stok tidak cukup untuk variant ${variant.sku}. Tersedia: ${currentStock}, Dibutuhkan: ${item.quantity}`);
                    }
            ...
            const statusNotificationMap = {
                paid: () => this.notificationService.notifyOrderPaid(order.user_id, order.id),
                shipped: () => this.notificationService.notifyOrderShipped(order.user_id, order.id),
                delivered: () => this.notificationService.notifyOrderDelivered(order.user_id, order.id),
                cancelled: () => this.notificationService.notifyOrderCancelled(order.user_id, order.id),
            };
